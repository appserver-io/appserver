...
class InitialContextTest extends \PHPUnit_Framework_TestCase {
	...
	public function testNewInstanceWithArgument() {
	    $configuration = $this->initialContext->newInstance('TechDivision\ApplicationServer\Configuration', array($nodeName = 'test'));
	...
	public function testLookupWithStatelessSessionBean() {
	    $singletonSessionBean = $this->initialContext->lookup('TechDivision\ApplicationServer\MockStatelessSessionBean', md5($time = time()));
	    $singletonSessionBean->setAValue($aValue = 10);
	    $this->assertEquals($aValue, $singletonSessionBean->getAValue());
	    $singletonSessionBean = $this->initialContext->lookup('TechDivision\ApplicationServer\MockStatelessSessionBean', md5($time));
	...
	public function testLookupWithSingletonSessionBean() {
	    ...
	    $persistentValue = 10;
	    $singletonSessionBean = $this->initialContext->lookup('TechDivision\ApplicationServer\MockSingletonSessionBean', md5($time = time()));
	    $singletonSessionBean->setPersistentValue($persistentValue);
	    $singletonSessionBean = $this->initialContext->lookup('TechDivision\ApplicationServer\MockSingletonSessionBean', md5(time() + 1));
	...
	public function testLookupWithStatefulSessionBean() {
	    ...
	    $persistentValue = 10;
	    $singletonSessionBean = $this->initialContext->lookup('TechDivision\ApplicationServer\MockStatefulSessionBean', md5($time = time()));
	    $singletonSessionBean->setPersistentValue($persistentValue);
	    $singletonSessionBean = $this->initialContext->lookup('TechDivision\ApplicationServer\MockStatefulSessionBean', md5($time));
	    $this->assertEquals($persistentValue, $singletonSessionBean->getPersistentValue());
	}
}