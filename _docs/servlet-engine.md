...
| `/web-app/session-config/garbage-collection-probability` | *float* | It allows you to set a value, how often the garbage collector is invoked. You can specify a value between `100` and `0`. The higher the value, the higher is the probability that the garbage collector will be invoked. With the number of decimals you extend the range, and therefore the probability that the GC will be invoked. By default the value for this node is set to `0.1`.
